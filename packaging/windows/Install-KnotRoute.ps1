...
$App = Join-Path $Source "KnotRoute.exe"
if (-not (Test-Path $App)) { throw (T "Place this script next to KnotRoute.exe." "Поместите этот скрипт рядом с KnotRoute.exe.") }
New-Item -ItemType Directory -Force $InstallDir | Out-Null
Copy-Item $App (Join-Path $InstallDir ".") -Force
$Shell = New-Object -ComObject WScript.Shell
$StartMenu = Join-Path $env:APPDATA "Microsoft\Windows\Start Menu\Programs\KnotRoute.lnk"
$Shortcut = $Shell.CreateShortcut($StartMenu)
$Shortcut.TargetPath = Join-Path $InstallDir "KnotRoute.exe"
...
Start-Process (Join-Path $InstallDir "KnotRoute.exe")
