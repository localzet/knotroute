...
$opsLdflags = "-s -w -X github.com/localzet/knotroute/internal/ops.Version=$Version"
...
try {
  foreach ($target in $targets) {
    ...
    try {
      ...
      go build -trimpath -ldflags=$opsLdflags -o (Join-Path $temp "knotroute-control$ext") ./cmd/knotroute-control
      go build -trimpath -ldflags=$opsLdflags -o (Join-Path $temp "knotroute-agent$ext") ./cmd/knotroute-agent
