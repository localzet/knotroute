...
function normalizeStatus(value: unknown): Status {
  ...
  return {
    ...
    known_services: asArray<any>(raw.known_services).map(service => ({ ...asObject(service), introduction_points: asArray<string>(asObject(service).introduction_points), metadata: asObject(asObject(service).metadata) })),
...
function nodeServiceDomain(service: string, nodeDomain: string) {
...
function renderRoutes(items: any[]) {
  ...
  root.innerHTML = `<table><thead><tr><th>Узел</th><th>Путь</th><th>Хопы</th><th>Объявленные сервисы</th></tr></thead><tbody>${items.map(r => `<tr><td><code title="${esc(r.destination)}">${esc(r.domain)}</code></td><td class="path">${r.path.map(short).map(esc).join(" → ")}</td><td>${r.hops}</td><td>${r.services.length ? r.services.map((x: string) => { const domain = nodeServiceDomain(x, r.domain); return `<a class="service-pill route-service-link" href="http://${esc(domain)}/" target="_blank" rel="noreferrer" title="Открыть ${esc(domain)}">${esc(x)}</a>`; }).join("") : "—"}</td></tr>`).join("")}</tbody></table>`;
}

function catalogEntries(s: any) {
  const entries: Array<{kind:string; domain:string; title:string; description:string; tags:string[]; href:string}> = [];
...
function renderCatalog(s: any) {
  const root = byId("catalog");
  const query = (byId<HTMLInputElement>("catalogSearch").value || "").trim().toLowerCase();
  ...
  if (!filtered.length) { root.className = "catalog-grid empty"; root.textContent = all.length ? "Ничего не найдено" : "Сервисы пока не обнаружены"; return; }
  ...
  for (const button of root.querySelectorAll<HTMLButtonElement>(".catalog-copy")) button.addEventListener("click", async () => { await navigator.clipboard.writeText(button.dataset.domain || ""); const old = button.textContent; button.textContent = "Скопировано"; setTimeout(() => button.textContent = old, 1000); });
}
...
function renderStatus(s: Status) {
  ...
  renderRoutes(s.routes); renderPeers(s.peers); renderEndpoints(s.services, s.forwards); renderCatalog(s); renderEvents(s.events);
...
byId<HTMLInputElement>("catalogSearch").addEventListener("input", () => { if (lastStatus) renderCatalog(lastStatus); });
